<div style="display:grid;gap:16px;">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <select name="user_id" required style="width:100%;padding:10px 14px;background:#0f0f14;border:1px solid rgba(255,255,255,0.1);border-radius:10px;color:white;">
                ...
                <option value="{{ $author->id }}" {{ old('user_id', $article->user_id ?? auth()->id()) == $author->id ? 'selected' : '' }}>{{ $author->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div>
        ...
        <textarea name="excerpt" rows="2" style="width:100%;padding:10px 14px;background:#0f0f14;border:1px solid rgba(255,255,255,0.1);border-radius:10px;color:white;">{{ old('excerpt', $article->excerpt ?? '') }}</textarea>
    </div>
    <div>
        <label style="display:block;color:#9ca3af;font-size:12px;font-weight:600;margin-bottom:6px;">Body</label>
        <textarea name="body" rows="12" required style="width:100%;padding:10px 14px;background:#0f0f14;border:1px solid rgba(255,255,255,0.1);border-radius:10px;color:white;">{{ old('body', $article->body ?? '') }}</textarea>
    </div>
    <div>
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6" style="display:flex;align-items:end;padding-bottom:8px;">
            <label style="color:#d1d5db;font-size:14px;cursor:pointer;">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $article->is_featured ?? false) ? 'checked' : '' }}> Featured article
            </label>
        </div>
    </div>
</div>
